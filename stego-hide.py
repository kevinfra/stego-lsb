...
def valid_text_length_image_size_and_admissible_bits(binary_text, usable_bits, k):
    if k < 2 or k > 8:
        ...
        return False
    if usable_bits - len(binary_text) - 4 - 32 <= 0:
...
def text_to_hide_with_padding(text, usable_bits, max_int):
    padding_size = usable_bits - len(text) - 4 - 32
    result = text
...
if __name__ == '__main__':
    ...
    text_length_and_admissible_bits_binary = '{0:032b}'.format(len(text_to_hide)) + '{0:04b}'.format(admissible_bits)
    header_from = 0
    header_to = 2
