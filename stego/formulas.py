...
def compare(first_image, second_image) -> Tuple[float,float,float]:
    first = Image.open(first_image)
    second = Image.open(second_image)
    w,h = first.size
    return MSE_Image(w,h,first,second)


def MSE_Image(width, height, original_image, output_image) -> Tuple[float,float,float]:
    sumTotalR = 0
    sumTotalG = 0
    sumTotalB = 0
    ...
    return (errorCuadraticoMedio, peakSignalToNoiseRatio, compare_ssim(original_image,output_image))
